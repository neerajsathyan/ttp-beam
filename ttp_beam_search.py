...
if __name__ == "__main__":
    ...
    f = open('data/beam_search/bs_results.csv', 'a')
    writer = csv.writer(f)
    typ = 'TSP'
    if 'cvrph' in sys.argv[4]:
        ...
        typ = 'CVRP'
    writer.writerow([md5(sys.argv[1].encode()).hexdigest() + "_" + str(uuid.uuid4()), sys.argv[1], ttp_instance.n, ttp_instance.streak_limit, ttp_instance.no_repeat, sys.argv[4], int(sys.argv[5]), bool(int(sys.argv[6])), bool(int(sys.argv[7])), float(sys.argv[8]), int(sys.argv[9]), terminal.shortest_path_length, time_elapsed])
    f.close()
